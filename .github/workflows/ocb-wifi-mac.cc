...
OcbWifiMac::ConfigureEdca (uint32_t cwmin, uint32_t cwmax, uint32_t aifsn, enum AcIndex ac)
{
  ...
  switch (ac)
    {
  case AC_DVO:
       dcf = RegularWifiMac::GetDVOQueue ();
    ...
    case AC_VO:
      ...
      break;
    case AC_KVI:
              dcf = RegularWifiMac::GetKVIQueue ();
    ...
    case AC_DVI:
          dcf = RegularWifiMac::GetDVIQueue ();
          dcf->SetMinCw ((cwmin + 1) / 2 - 1);
          dcf->SetMaxCw (cwmin);
          dcf->SetAifsn (aifsn);
          break;
...
OcbWifiMac::FinishConfigureStandard (enum WifiPhyStandard standard)
{
  ...
  ConfigureEdca (cwmin, cwmax, 1, AC_DVO);
  ConfigureEdca (cwmin, cwmax, 2, AC_VO);
  ConfigureEdca (cwmin, cwmax, 3, AC_KVI);
  ConfigureEdca (cwmin, cwmax, 4, AC_DVI);
  ConfigureEdca (cwmin, cwmax, 5, AC_VI);
  ConfigureEdca (cwmin, cwmax, 6, AC_BE);
  ConfigureEdca (cwmin, cwmax, 7, AC_BK);
}
